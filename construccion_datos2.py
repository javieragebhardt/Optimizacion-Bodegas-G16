...
def generar_t(bdd, inferior, superior):
    bdd_copy = bdd.copy()
    bdd_copy['Horas'] = 48
    clientes_con_menos_del_25_percent = bdd['Cantidad'][bdd['Cantidad'] <= bdd['Cantidad'].max() * inferior]
    clientes_con_entre_25y75_percent =  bdd['Cantidad'][( bdd['Cantidad'] >  bdd['Cantidad'].max() * inferior ) & ( bdd['Cantidad'] <=  bdd['Cantidad'].max() * superior)]
    clientes_con_sobre_75_percent =  bdd['Cantidad'][bdd['Cantidad'].max() * superior < bdd['Cantidad']]
    bdd_copy.loc[bdd_copy['ID Cliente'].isin(clientes_con_entre_25y75_percent.index), 'Horas'] = 24
    bdd_copy.loc[bdd_copy['ID Cliente'].isin(clientes_con_sobre_75_percent.index), 'Horas'] = 12 
    cantidad_por_categoria = bdd_copy['Horas'].value_counts()
    print(cantidad_por_categoria)


generar_t(bdd_ventas_agrupadas, 0.25, 0.75)
generar_t(bdd_ventas_agrupadas, 0.1, 0.8)
